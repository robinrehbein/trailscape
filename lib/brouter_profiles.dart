...
///
/// Der öffentliche BRouter-Server (brouter.de) bietet kein festes
/// Gravel-Profil an, erlaubt aber das Hochladen eigener Profile. Das hier
/// eingebettete Profil ist das offizielle `gravel.brf` (Version 28.04.2024,
/// von quaelnix) aus dem BRouter-Repository (abrensch/brouter) und wird
/// unverändert übernommen.
library;

/// Das offizielle BRouter-Gravel-Profil, wortwörtlich eingebettet.
const String gravelBrf = r'''
...
/// Zeile im Profil, die die Unpaved-Präferenz steuert (Auslieferungszustand).
const String _preferUnpavedOff = 'assign prefer_unpaved_paths false';

/// Dieselbe Zeile mit aktivierter Unpaved-Präferenz.
const String _preferUnpavedOn = 'assign prefer_unpaved_paths true';

/// Liefert den Profiltext für die Weg-Präferenz "Schotter & Kieswege".
///
/// Basiert auf [gravelBrf], schaltet aber `prefer_unpaved_paths` auf `true`,
/// damit unbefestigte Wege (Schotter, Kies, Waldwege) bevorzugt werden.
String gravelProfileText() {
  if (!gravelBrf.contains(_preferUnpavedOff)) {
    throw StateError(
      'Konnte "$_preferUnpavedOff" im eingebetteten Gravel-Profil nicht '
      'finden - das Profil hat sich vermutlich geändert.',
    );
  }
  final text = gravelBrf.replaceFirst(_preferUnpavedOff, _preferUnpavedOn);
  if (!text.contains(_preferUnpavedOn) || text.contains(_preferUnpavedOff)) {
    throw StateError(
      'Ersetzen von "$_preferUnpavedOff" im eingebetteten Gravel-Profil '
      'fehlgeschlagen.',
    );
  }
  assert(text.length == gravelBrf.length - 1);
  return text;
}
