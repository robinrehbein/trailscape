...
/**
 * Eingebettete BRouter-Custom-Profile.
 *
 * 1:1-Portierung von `lib/brouter_profiles.dart`. Der oeffentliche
 * BRouter-Server (brouter.de) bietet kein festes Gravel-Profil an, erlaubt
 * aber das Hochladen eigener Profile. Das hier eingebettete Profil ist das
 * offizielle `gravel.brf` (Version 28.04.2024, von quaelnix) aus dem
 * BRouter-Repository (abrensch/brouter) und wird unveraendert uebernommen.
 */

/**
 * Das offizielle BRouter-Gravel-Profil, wortwoertlich eingebettet.
 *
 * Kotlins Raw-Strings behalten (anders als Darts Multiline-Strings) einen
 * unmittelbar auf die oeffnenden Anfuehrungszeichen folgenden Zeilenumbruch
 * bei — deshalb beginnt der Inhalt hier bewusst auf derselben Zeile wie
 * `"""`, damit der Wert byte-identisch zum Original bleibt (inkl. des
 * fuehrenden `#` und des abschliessenden Zeilenumbruchs vor dem `0`).
 */
const val GRAVEL_BRF = """# "gravel.brf" -- Version 28.04.2024
...
/** Zeile im Profil, die die Unpaved-Praeferenz steuert (Auslieferungszustand). */
private const val PREFER_UNPAVED_OFF = "assign prefer_unpaved_paths false"

/** Dieselbe Zeile mit aktivierter Unpaved-Praeferenz. */
private const val PREFER_UNPAVED_ON = "assign prefer_unpaved_paths true"

/**
 * Liefert den Profiltext fuer die Weg-Praeferenz "Schotter & Kieswege".
 *
 * Basiert auf [GRAVEL_BRF], schaltet aber `prefer_unpaved_paths` auf `true`,
 * damit unbefestigte Wege (Schotter, Kies, Waldwege) bevorzugt werden.
 */
fun gravelProfileText(): String {
    if (!GRAVEL_BRF.contains(PREFER_UNPAVED_OFF)) {
        throw IllegalStateException(
            "Konnte \"$PREFER_UNPAVED_OFF\" im eingebetteten Gravel-Profil nicht " +
                "finden - das Profil hat sich vermutlich geaendert.",
        )
    }
    val text = GRAVEL_BRF.replaceFirst(PREFER_UNPAVED_OFF, PREFER_UNPAVED_ON)
    if (!text.contains(PREFER_UNPAVED_ON) || text.contains(PREFER_UNPAVED_OFF)) {
        throw IllegalStateException(
            "Ersetzen von \"$PREFER_UNPAVED_OFF\" im eingebetteten Gravel-Profil " +
                "fehlgeschlagen.",
        )
    }
    check(text.length == GRAVEL_BRF.length - 1)
    return text
}
